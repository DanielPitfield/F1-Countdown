...
export const driverRouter = router({
  ...
  getChampionshipResults: publicProcedure.input(z.object({ driverID: z.string().min(1).trim() })).query(
    async ({
      input,
    }): Promise<{
  ...
  getRaces: publicProcedure.input(z.object({ driverID: z.string().min(1).trim() })).query(
    async ({
      input,
    }): Promise<{
      ...
      const winningRaces: Race[] = allRaces.filter((race) => {
        ...
        const finishingPosition: string =
          race.Results.find((result) => result.Driver.driverId === input.driverID)?.position ?? "";
  ...
  getPolePositions: publicProcedure.input(z.object({ driverID: z.string().min(1).trim() })).query(
    async ({
      input,
    }): Promise<{
      firstPole: Race;
