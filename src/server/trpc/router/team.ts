...
export const teamRouter = router({
  ...
  getChampionshipResults: publicProcedure.input(z.object({ teamID: z.string().min(1).trim() })).query(
    async ({
      input,
    }): Promise<{
      ...
      const allYears: TeamSeasonResult[] = data.MRData.StandingsTable.StandingsLists.map(
  ...
  getRaces: publicProcedure.input(z.object({ teamID: z.string().min(1).trim() })).query(
    async ({
      input,
    }): Promise<{
  ...
  getPolePositions: publicProcedure.input(z.object({ teamID: z.string().min(1).trim() })).query(
    async ({
      input,
    }): Promise<{
      firstPole: Race;
